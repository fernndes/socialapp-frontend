        ...
        <Grid container className={classes.form}>
            ...
            <Grid item xs={12} sm={8} md={5} className={classes.container}>
                ...
                <form onSubmit={handleSubmit}>
                    <TextField id="email" name="email" type="email" label="E-mail" className={classes.textField} value={email} onChange={(event) => setEmail(event.target.value)} helperText={errors && (errors.email || errors.error) ? (errors.error ? errors.error : errors.email.text) : "Digite o seu e-mail"} error={errors && (errors.email || errors.error) ? true : false} fullWidth required={true} autoComplete="off" inputProps={{ minLength: 1 }} />
                    <TextField id="password" name="password" type="password" label="Senha" className={classes.textField} value={password} onChange={(event) => setPassword(event.target.value)} fullWidth required helperText={errors && errors.password ? errors.password.text : "A senha deve conter entre 8 e 30 caracteres"} error={errors && errors.password ? true : false} autoComplete="off" inputProps={{ minLength: 8, maxLength: 30 }} />
                    <TextField id="confirmPassword" name="confirmPassword" type="password" label="Confirme sua senha" className={classes.textField} value={confirmPassword} onChange={(event) => setConfirmPassword(event.target.value)} fullWidth required helperText={errors && errors.confirmPassword ? errors.confirmPassword.text : "As senhas devem ser equivalentes"} error={errors && errors.confirmPassword ? true : false} autoComplete="off" inputProps={{ minLength: 8, maxLength: 30 }} />
                    <TextField id="username" name="username" type="username" label="Nome de usuário" className={classes.textField} value={username} onChange={(event) => setUsername(event.target.value)} fullWidth required helperText={errors && errors.username ? (errors.username.text || errors.username) : "O nome de usuário deve conter entre 6 e 12 caracteres"} error={errors && errors.username ? true : false} autoComplete="off" inputProps={{ minLength: 6, maxLength: 12 }} />
                    {!loading ? (
